...
```
aws athena start-query-execution \
--query-string "CREATE table default.noaatmpparquet WITH (format='PARQUET', external_location='s3://[YOUR PRIVATE S3 BUCKET]/parquet/') AS SELECT * FROM default.noaatmp WHERE q_flag = '';" \
--region us-east-1
```

*Filter out the dataset for 2019 results on only the NY weather groundstations*
```
last_query_id="$(aws athena start-query-execution \
--query-string "SELECT * FROM default.noaatmpparquet WHERE id IN ('US1NYNY0074', 'USC00305798', 'USC00305799', 'USC00305804', 'USC00305806', 'USC00305816', 'USW00014732', 'USW00014786', 'USW00093732', 'USW00094728', 'USW00094789') AND year_date LIKE '2019%';" \
--result-configuration "OutputLocation=s3://aws-athena-query-results-538473421153-us-east-1/Unsaved/2019/06/07/" \
--work-group "primary" \
--region us-east-1 \
